...
pub fn Shard(comptime ItemPtr: type) type {
    return struct {
        ...
        pub fn get(self: *Self, key: []const u8) ?ItemPtr {
            self.lock.lockShared();
            defer self.lock.unlockShared();
            return self.map.get(key);
        }

        pub fn set(self: *Self, allocator: std.mem.Allocator, key: []const u8, item: ItemPtr) !?ItemPtr {
            self.lock.lock();
            defer self.lock.unlock();
        ...
        pub fn delete(self: *Self, allocator: std.mem.Allocator, key: []const u8) ?ItemPtr {
            self.lock.lock();
            defer self.lock.unlock();
        ...
        pub fn sampleAtRetained(self: *Self, idx: usize) ?ItemPtr {
            self.lock.lockShared();
            defer self.lock.unlockShared();
        ...
        pub fn snapshot(self: *Self, allocator: std.mem.Allocator, out: *std.ArrayList(ItemPtr)) !void {
            self.lock.lockShared();
            defer self.lock.unlockShared();
